...
def canonical_json(value: Any) -> str:
    ...
    def clean(v: Any) -> Any:
        ...
        if isinstance(v, dict): return {str(k): clean(x) for k, x in sorted(v.items()) if str(k).lower() not in {"api_token", "access_token", "secret", "password", "prompt", "hidden_states"}}
...
def model_result_from_json(payload: str) -> MAMVModelResult:
    ...
    for k in ("visible_source_ids","excluded_source_ids","assumptions","limitations","warnings"): fd[k]=tuple(fd.get(k,()))
    ...
    d["inference_frame"]=_construct(InferenceFrame,fd); d["warnings"]=tuple(FrameWarning(**({**x,"source_ids":tuple(x.get("source_ids",()))} if isinstance(x,dict) else {"code":"GROUNDING_SCOPE_LIMITED","message":x,"affected_field":"legacy","severity":"warning"})) for x in d.get("warnings",()))
    ...
    for k,c in (("claim_candidates",ClaimCandidate),("evidence_candidates",EvidenceCandidate),("proposed_relations",ProposedEvidenceRelation)):
